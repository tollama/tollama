...
Tollama는 파편화된 TSFM(Time Series Foundation Model)들을 **Ollama 스타일의 통합 인터페이스**로 묶어주는 로컬 우선(local-first) 예측 플랫폼이다. FastAPI 데몬 + CLI + Python SDK + 웹/TUI 대시보드 + MCP/A2A 에이전트 연동까지 갖춘 **풀스택 시계열 예측 플랫폼**으로 구현되어 있다.

현재 HTTP 엔드포인트 인벤토리는 `docs/api-reference.md`, 에이전트 도구 표면은
`docs/agent-tools.md`, 모델/패밀리 범위는 `docs/models.md`와
`model-registry/registry.yaml`이 기준 문서다. 이 문서는 숫자 인벤토리보다
구조적 강점과 개선 포인트를 중심으로 본다.
...
┌──────────────────────────────────────────────────────────────┐
│              에이전트 / 프레임워크 연동 레이어                   │
│  MCP Server  │  A2A (JSON-RPC)  │  OpenClaw Skill │
...
│  │ /api/validate  /api/usage  /metrics (Prometheus)      │   │
│  └───────────────────────────────────────────────────────┘   │
│  ┌─ 모델 관리 ───────────────────────────────────────────┐   │
...
현재 지원 범위의 canonical inventory는 `docs/models.md`와
`model-registry/registry.yaml`에 유지된다. 이 review 문서에서는 대표
패밀리와 capability 성격만 요약한다.

| 대표 패밀리 | 예시 모델 | Covariates 지원 |
|------|------------|----------------|
| `mock` | Mock (테스트용) | 테스트 목적 전체 지원 |
| `torch` | Chronos-2, Granite TTM R2 | Past + Future |
| `timesfm` | TimesFM 2.5-200M | Past + Future |
| `uni2ts` | Moirai 2.0-R Small | Past + Future |
| `sundial`, `toto` | Sundial Base 128M, Toto Open Base 1.0 | Target only / Past only |
| 추가 family | `lag_llama`, `patchtst`, `tide`, `nhits`, `nbeatsx`, `timer`, `timemixer`, `forecastpfn` | 모델별 상이 |
...
| 기능 | 상태 | 설명 |
|------|:----:|------|
| CLI 라이프사이클 | ✅ | pull, run, list, show, ps, rm, info, doctor, quickstart, explain, open |
| FastAPI HTTP API | ✅ | 시스템/모델/예측/분석/인제스트/프로필/대시보드/A2A 라우트 제공. canonical inventory는 `docs/api-reference.md` |
| HuggingFace Pull | ✅ | NDJSON 스트리밍 진행률, 오프라인 모드, 라이선스 수락 처리 |
| 런타임 격리 | ✅ | 패밀리별 자동 venv 부트스트랩, Python 버전 제약 관리 |
| Covariates 통합 | ✅ | registry-backed capability matrix와 `best_effort` / `strict` 계약 |
| 프록시/토큰 설정 | ✅ | config, env, CLI 플래그 모두 지원 |
| 진단 엔드포인트 | ✅ | `/api/info`, `tollama info`, `tollama doctor` |
| 테스트/CI | ✅ | Ruff + Pytest + GitHub Actions 기반 단위/통합 회귀 |
| Python SDK | ✅ | `Tollama` 클래스 — forecast, compare, what_if, analyze, generate, report, workflow API, DataFrame 변환 |
...
| MCP Server | ✅ | FastMCP 기반 forecast / analyze / compare / what-if / report / recommend 계열 도구 표면 |
...
| 프레임워크 연동 | ✅ | LangChain, CrewAI, AutoGen, Smolagents 어댑터 |
...
├── core/            # 공유 계약 + deterministic domain services
...
PEP 621 패키징, src 레이아웃, Ruff 린팅, Pytest, CI — 오픈소스 표준을 잘 따르고 있다. 테스트와 문서가 모두 비교적 폭넓게 유지되고 있다.
...
MCP Server, A2A 프로토콜(JSON-RPC + 태스크 큐), LangChain, CrewAI, AutoGen, Smolagents 어댑터, OpenClaw 스킬까지 — 주요 AI 에이전트 프레임워크를 거의 모두 커버한다. AI 에이전트가 시계열 예측을 "도구"로 사용할 수 있는 인프라가 이미 완비되어 있다.
...
- [x] 자동 모델 추천 (`/api/recommend`, `auto_select.py`)
- [x] MCP Server
- [x] Dockerfile 작성
- [x] A2A 프로토콜 (JSON-RPC)
...
**현재 상태**: Tollama는 핵심 아키텍처(데몬 + 런타임 격리 + CLI)를 넘어 **Python SDK, 이중 대시보드(웹+TUI), MCP Server, A2A 프로토콜, 주요 에이전트 프레임워크 연동, 고급 분석(What-if, 반사실적, 시나리오 트리, 파이프라인, 리포트)**까지 갖춘 **기능적으로 성숙한 시계열 예측 플랫폼**이다. 초기 비전에서 제시한 SDK, Dashboard, MCP, A2A 등이 모두 구현되어 있다.
