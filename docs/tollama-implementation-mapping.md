...
현재 HTTP 엔드포인트 인벤토리는 `docs/api-reference.md`, 에이전트 도구 표면은
`docs/agent-tools.md`, 모델/패밀리 범위는 `docs/models.md`와
`model-registry/registry.yaml`이 기준 문서입니다. 이 문서는 숫자 카운트보다
기획 의도와 구현 연결에 집중합니다.
...
| 인터페이스 | 설명 |
|-----------|------|
| **CLI** | `tollama serve`, `tollama pull`, `tollama run`, `tollama list`/`show`/`ps`/`rm` 중심의 Ollama-style lifecycle |
| **HTTP API** | `/api/*`(Ollama 스타일) + `/v1/*`(stable). 기준 인벤토리는 `docs/api-reference.md` |
| **Python SDK** | `Tollama` 클래스 기반 workflow 체이닝 + DataFrame 변환 |
...
| 연동 경로 | 설명 |
|----------|------|
| **MCP Server** (`tollama-mcp`) | Claude Desktop/Code에서 네이티브 도구로 연결 |
| **LangChain** | `get_tollama_tools(...)` 래퍼 |
| **CrewAI** | `get_crewai_tools(...)` |
| **AutoGen** | `get_autogen_tool_specs()` + `register_autogen_tools()` |
| **smolagents** | `get_smolagents_tools(...)` |
| **A2A** (JSON-RPC) | `/.well-known/agent-card.json` 디스커버리 + `/a2a` 호출 + SSE 스트리밍 |
| **OpenClaw Skill** | `skills/tollama-forecast/` — 표준 exit code 계약(v2) |
...
registry-backed runner family가 "모델 패밀리" 단위로 분리되어 있다.
대표 family는 `mock`, `torch`, `timesfm`, `uni2ts`, `sundial`, `toto`,
`lag_llama`, `patchtst`, `tide`, `nhits`, `nbeatsx`, `timer`,
`timemixer`, `forecastpfn`이다.
...
- **레지스트리**: `model-registry/registry.yaml`이 machine-readable 모델 catalog 역할 수행
...
| `POST /api/explain-decision` | 예측 + 신뢰/정책 컨텍스트를 묶은 설명 facade |
| `POST /api/reconcile` | 계층형 예측 조정(bottom-up, top-down, OLS, MinT) |
| `POST /api/conformal` | conformal interval 보정 |

> **참고**: 모델 추천 로직 자체는 `core/auto_select.py`, `core/recommend.py`, `/api/auto-forecast`,
> `/api/pipeline`, MCP/A2A 경로에서 사용되며, 현재 별도 `POST /api/recommend` REST 라우트는 없다.
...
현재 지원 모델과 패밀리의 canonical inventory는 `docs/models.md`와
`model-registry/registry.yaml`이 맡는다. 이 문서에서는 "registry-backed
catalog + family-specific runner isolation"이라는 구조만 강조한다.

- 모델은 레지스트리 이름으로 직접 관리된다.
- 설치/삭제/조회는 모두 동일한 lifecycle(`pull`/`show`/`list`/`ps`/`rm`)로 접근한다.
- family별 runtime extra와 부트스트랩 정보는 `pyproject.toml`과
  `src/tollama/core/runtime_bootstrap.py`가 기준이다.
- `uni2ts`와 `timesfm`처럼 Python 제약이 있는 family는 runtime bootstrap 단계에서 명시적으로 검증된다.
...
현재 MCP 도구 표면은 아래 카테고리를 커버하며, 정확한 등록 목록은
`src/tollama/mcp/server.py`가 기준이다.

- 시스템: `health`, `models`, `show`, `pull`
- 예측: `forecast`, `auto_forecast`, `compare`
- 분석: `analyze`, `generate`, `recommend`
- 시나리오/오케스트레이션: `what_if`, `counterfactual`, `scenario_tree`, `pipeline`, `report`

**XAI / trust tools (7개):**

| 도구 | 설명 |
|------|------|
| `tollama_explain` | trust-aware 의사결정 설명 |
| `tollama_trust_score` | trust score breakdown 계산 |
| `tollama_model_card` | EU AI Act 스타일 model card 생성 |
| `tollama_gate_decision` | trust gate 정책 평가 |
| `tollama_batch_analyze` | 배치 trust 분석 |
| `tollama_alerts_configure` | trust alert threshold 설정 |
| `tollama_alerts_check` | threshold 기준 alert 평가 |

### LangChain / CrewAI / AutoGen / smolagents

| 프레임워크 | 진입점 | 구현 형태 |
|-----------|--------|-----------|
| **LangChain** | `get_tollama_tools(base_url, timeout)` | Tool 클래스 래퍼 |
...
│      ▼       ▼       ▼       ▼       ▼       ▼               │
│    family-specific runners (mock, torch, timesfm, ...)      │
