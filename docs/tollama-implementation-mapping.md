...
| 인터페이스 | 설명 |
|-----------|------|
| **CLI** | `serve`/`pull`/`run`/`list`/`show`/`ps`/`rm` 중심 플로우에 `runtime`/`routing`/`config`/`modelfile`/`dev`/`xai` 서브커맨드까지 포함 |
| **HTTP API** | `/api/*`(Ollama 스타일) + `/v1/*` + dashboard/XAI/A2A 라우트로 예측부터 운영/설명까지 제공 |
| **Python SDK** | `Tollama` 클래스 + workflow 체이닝 + DataFrame 변환 + 파일/시나리오/분석 헬퍼 |
...
| 연동 경로 | 도구 수 | 설명 |
|----------|:------:|------|
| **MCP Server** (`tollama-mcp`) | 22개 | Claude Desktop/Code에서 네이티브 도구로 연결 (forecast/orchestration 15 + XAI/trust 7) |
...
`mock`, `torch`, `timesfm`, `uni2ts`, `sundial`, `toto`, `lag_llama`, `patchtst`,
`tide`, `nhits`, `nbeatsx`, `timer`, `timemixer`, `forecastpfn` 런너가 "모델 패밀리"
단위로 분리되어 있다.
...
- **레지스트리**: `model-registry/registry.yaml`에 `mock` 포함 15개 엔트리 선언
...
| `lag-llama` | `lag_llama` | `time-series-foundation-models/Lag-Llama` | Apache-2.0 | No | `runner_lag_llama` |
| `patchtst` | `patchtst` | `ibm-granite/granite-timeseries-patchtst` | Apache-2.0 | No | `runner_patchtst` |
| `tide` | `tide` | `tollama/tide-runner` | Apache-2.0 | No | `runner_tide` |
| `nhits` | `nhits` | `tollama/nhits-runner` | Apache-2.0 | No | `runner_nhits` |
| `nbeatsx` | `nbeatsx` | `tollama/nbeatsx-runner` | Apache-2.0 | No | `runner_nbeatsx` |
| `timer-base` | `timer` | `thuml/timer-base-84m` | Apache-2.0 | No | `runner_timer` |
| `timemixer-base` | `timemixer` | `thuml/timemixer` | Apache-2.0 | No | `runner_timemixer` |
| `forecastpfn` | `forecastpfn` | `abacusai/ForecastPFN` | Apache-2.0 | No | `runner_forecastpfn` |
...
**Core forecasting/orchestration tools (15개):**
...
┌──────────────────────────────────────────────────────────────┐
│                        사용자 접점                             │
│  CLI / SDK / HTTP API / Web + TUI Dashboard                   │
│  Web Dashboard (Alpine.js + HTMX + Chart.js)                 │
│  TUI Dashboard (Textual)                                     │
├──────────────────────────────────────────────────────────────┤
│                    에이전트 연동 레이어                         │
│  MCP Server (22 tools)  │  A2A (JSON-RPC + SSE)              │
