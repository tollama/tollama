            ...
            try:
                ...
                if entry.get("status") == "pass":
                    ...
                    for i, r_entry in enumerate(data["entries"]):
                        if (
                            r_entry.get("model") == entry.get("model")
                            and r_entry.get("dataset") == entry.get("dataset")
                            and r_entry.get("scenario") == "benchmark_target_only"
                        ):
                            # Multiple series per dataset; match the correct one.
                            # result.json entries do not store a series ID.
                            # raw/ and result.json are 1:1 for each series in order.
                            # Recompute metrics from raw files and rebuild result.json entries.
                            pass
            except Exception:
                pass
...
# Better approach: read all raw files, compute metrics, then rebuild result.json
# entries from raw files.
entries = []
for model_dir in Path("artifacts/realdata/hf-local/raw").iterdir():
    if not model_dir.is_dir():
        continue
        ...
        for scenario_dir in dataset_dir.iterdir():
            ...
            for entry_file in scenario_dir.glob("*.json"):
                ...
                if (
                    ...
                    and entry.get("scenario") == "benchmark_target_only"
                ):
                    actuals = entry_data["request"]["series"][0]["actuals"]
                    preds = entry_data["response"]["forecasts"][0]["mean"]
                    history = entry_data["request"]["series"][0]["target"]
                    ...
                    if "metrics" not in entry:
                        entry["metrics"] = {}
                    if mape is not None:
                        entry["metrics"]["mape"] = mape
                    if mase is not None:
                        entry["metrics"]["mase"] = mase
