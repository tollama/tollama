...
def test_compute_forecast_metrics_returns_mape_and_mase_with_macro_aggregate() -> None:
    request = ForecastRequest.model_validate(
        {
            ...
            "options": {},
            "parameters": {"metrics": {"names": ["mape", "mase"]}},
        },
    )
    ...
    assert metrics.series[0].values == pytest.approx({"mape": 37.5, "mase": 0.5})
    assert metrics.series[1].id == "s2"
    assert metrics.series[1].values == pytest.approx({"mape": 50.0, "mase": 0.5})
