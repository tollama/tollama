"""Uni2TS runner process using newline-delimited JSON over stdio."""
...
def _handle_hello(request: ProtocolRequest) -> ProtocolResponse:
    return ProtocolResponse(
        ...
        result={
            ...
            "supported_families": ["uni2ts"],
        },
    )


def _handle_unload(request: ProtocolRequest, adapter: MoiraiAdapter) -> ProtocolResponse:
...
def _handle_forecast(request: ProtocolRequest, adapter: MoiraiAdapter) -> ProtocolResponse:
    canonical_params = {
...
def handle_request_line(line: str | bytes, adapter: MoiraiAdapter) -> ProtocolResponse:
