"""ForecastPFN runner process using newline-delimited JSON over stdio."""
...
def _error_response(
    request_id: str, code: int | str, message: str, data: Any | None = None,
) -> ProtocolResponse:
...
def _handle_hello(request: ProtocolRequest) -> ProtocolResponse:
    return ProtocolResponse(
        ...
        result={
            ...
            "supported_families": ["forecastpfn"],
        },
    )


def _handle_unload(request: ProtocolRequest, adapter: ForecastPFNAdapter) -> ProtocolResponse:
    ...
    return ProtocolResponse(id=request.id, result={"unloaded": True, "model": model_name})


def _handle_forecast(request: ProtocolRequest, adapter: ForecastPFNAdapter) -> ProtocolResponse:
    canonical_params = {
    ...
    try:
        ...
        return _error_response(request.id, code="FORECAST_ERROR", message=msg)
...
def handle_request_line(line: str | bytes, adapter: ForecastPFNAdapter) -> ProtocolResponse:
    ...
    return _error_response(
        request.id, code=-32601, message="method not found",
