"""Timer runner process using newline-delimited JSON over stdio."""
...
def _handle_hello(request: ProtocolRequest) -> ProtocolResponse:
    return ProtocolResponse(
        ...
        result={
            ...
            "supported_families": ["timer"],
        },
    )


def _handle_unload(request: ProtocolRequest, adapter: TimerAdapter) -> ProtocolResponse:
...
def _handle_forecast(request: ProtocolRequest, adapter: TimerAdapter) -> ProtocolResponse:
    canonical_params = {
...
def handle_request_line(line: str | bytes, adapter: TimerAdapter) -> ProtocolResponse:
