"""TimeMixer runner process using newline-delimited JSON over stdio."""
...
def _handle_hello(request: ProtocolRequest) -> ProtocolResponse:
    return ProtocolResponse(
        ...
        result={
            ...
            "supported_families": ["timemixer"],
        },
    )


def _handle_unload(request: ProtocolRequest, adapter: TimeMixerAdapter) -> ProtocolResponse:
...
def _handle_forecast(request: ProtocolRequest, adapter: TimeMixerAdapter) -> ProtocolResponse:
    canonical_params = {
...
def handle_request_line(line: str | bytes, adapter: TimeMixerAdapter) -> ProtocolResponse:
