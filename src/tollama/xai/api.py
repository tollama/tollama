...
class ExplainDecisionRequest(BaseModel):
    ...
    eval_result: dict[str, Any] | None = Field(None, description="Output from tollama-eval")
    ...
    policy_config: dict[str, Any] | None = Field(None, description="Decision policy configuration")
...
class ExplainDecisionResponse(BaseModel):
    ...
    metadata: dict[str, Any] = Field(..., description="Additional metadata about the explanation")
...
class TrustBreakdownRequest(BaseModel):
    ...
    signals: list[dict[str, Any]] | None = Field(None, description="Multiple signals")
...
class ModelCardRequest(BaseModel):
    ...
    model_info: dict[str, Any] = Field(..., description="Model identity information")
...
class DecisionReportRequest(BaseModel):
    ...
    explanation: dict[str, Any] = Field(..., description="Output from /api/explain-decision")
...
class AlertCheckResponse(BaseModel):
    ...
    trust_result: dict[str, Any] | None = Field(
        None,
        description="Trust analysis result summary.",
    )
...
async def dashboard_history(request: TrustHistoryRequest):
    ...
    for domain in domains:
        entry: dict[str, Any] = {
            "history": [
                r.model_dump(mode="json") for r in tracker.get_history(domain, limit=request.limit)
            ],
        }
...
class AlertThreshold(BaseModel):
    ...
    webhook_url: str | None = Field(
        None,
        description="URL to POST alert payload. Omit for log-only.",
    )
...
async def check_alerts(request: AlertCheckRequest, raw_request: Request):
    ...
    for threshold in _alert_thresholds:
        ...
        if result.trust_score < threshold.min_trust_score:
            reasons.append(
                f"trust_score {result.trust_score:.2f} < threshold {threshold.min_trust_score:.2f}"
            )
